...
def test_model(rank, model, test_data, device='cpu', criterion=nn.NLLLoss(), tokenizer=None):
    ...
    logging.info(f"testing model for client {rank}")
    test_loss = 0
    correct = 0
    top_5 = 0
    ...
    with torch.no_grad():
        ...
        #for data, target, index in test_data:
        #for client, data, target, index in test_data: #extracting client id during testing
        for data, target, index, client, fetched_from_cache in test_data: #extracting client id during testing
            try:
                if args.task == 'nlp':
                    ...
                    #data, target, index = Variable(data).to(device=device), Variable(target).to(device=device), Variable(index).to(device=device)
                    # client, data, target, index = Variable(client).to(device=device), Variable(data).to(device=device), Variable(target).to(device=device), Variable(index).to(device=device)
                    data, target, index, client, fetched_from_cache = Variable(data).to(device=device), Variable(target).to(device=device), Variable(index).to(device=device), Variable(client).to(device=device),  Variable(fetched_from_cache).to(device=device)

                    #print(f"type(data) in test: {type(data)}")
