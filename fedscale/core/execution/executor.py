...
class Executor(object):
    ...
    def init_data(self):
        ...
        return training_sets, testing_sets #self.training_sets or self.testing_sets is just a 2D-array. for ex. [[1,2,3],[4,5,6], ..] which means client with unique id(i.e. index here) 0 has samples 1,2,3 and client with unique id 1 has samples 4,5,6

    def run(self):
        """Start running the executor by setting up execution and communication environment, and monitoring the grpc message.
        """
        self.setup_env()
        self.training_sets, self.testing_sets = self.init_data() #self.training_sets is just a 2D-array. for ex. [[1,2,3],[4,5,6], ..] which means client with unique id(i.e. index here) 0 has samples 1,2,3 and client with unique id 1 has samples 4,5,6
        self.indices = {}
        self.weights = {}
        self.check = {}
    ...
    def training_handler(self, clientId, conf, model=None):
        ...
        if self.args.task == "rl":
            ...
            client_data = select_dataset(clientId, self.training_sets,
                                         ...
                                         indices = self.indices[clientId] if clientId in self.indices else None,
                                         weights = self.weights[clientId] if clientId in self.weights else None
                                         ) #self.training_sets is just a 2D-array. for ex. [[1,2,3],[4,5,6], ..] which means client with unique id(i.e. index here) 0 has samples 1,2,3 and client with unique id 1 has samples 4,5,6

            client = self.get_client_trainer(conf)
            train_res = client.train(
                client_data=client_data, model=client_model, conf=conf)

            # if clientId in self.check:
            #     self.check[clientId] +=1
            #     print(f"check clientId: {clientId} : {self.check[clientId]}")
            # else:
            #     self.check[clientId] = 1
            #self.indices[clientId] = train_res['all_indices']
            #self.weights[clientId] = train_res['all_weights']

            #logging.info(f" trainhandler {clientId}:  ")
    ...
    def testing_handler(self, args, config=None):
        ...
        if self.task == 'rl':
            ...
            data_loader = select_dataset(self.this_rank, self.testing_sets,
                                         ...
                                         isTest=True, collate_fn=self.collate_fn, indices = None, weights = None
                                         )
