...
class clientManager(object):
    ...
    def register_client(self, hostId: int, clientId: int, size: int, speed: Dict[str, float], duration: float=1) -> None:
        ...
        user_trace = None if self.user_trace is None else self.user_trace[self.user_trace_keys[int(
            clientId) % len(self.user_trace)]] #this is the behave trace found in client_behave_trace.csv file

        # print("user_trace: ", self.user_trace)
        # print("len(user_trace): ", len(self.user_trace))
        # print("user_trace_keys: ", self.user_trace_keys)
        # print("len(user_trace_keys): ", len(self.user_trace_keys) )

        self.Clients[uniqueId] = Client(hostId, clientId, speed, user_trace)

        # remove clients
        if size >= self.filter_less and size <= self.filter_more: #if number of samples of the client within this range
    ...
    def select_participants(self, num_of_clients: int, cur_time: float=0) -> List[int]:
        ...
        if self.mode == "oort" and self.count > 1:
            ...
            #client_len = max(num_of_clients, len(clients_online) - 1)
            pickled_clients = clients_online[:client_len]
        #client selection randomize at this place.
        self.rng.shuffle(clients_online)
        client_len = min(num_of_clients, len(clients_online) - 1)
        randomly_shuffled_clients = clients_online[:client_len]

        print(f"clients needed: {num_of_clients}, clients selected by pol: {len(pickled_clients)}, clients selected randomly: {len(randomly_shuffled_clients)}")
        return pickled_clients, randomly_shuffled_clients
